...
void clearStack(OS_TCB *task)
{
    ...
    int i;
    /*CPU_STK *p_sp = task->StkBasePtr;
    p_sp = task->StkBasePtr;
    for (i = 0u; i < task->StkSize; i++) {               // Stack grows from HIGH to LOW memory                    
        *p_sp = (CPU_STK)0;                         // Clear from bottom of stack and up!                     
        p_sp++;
    }

    p_sp = OSTaskStkInit(task->TaskEntryAddr,
                         task->TaskEntryArg,
                         task->StkBasePtr,
                         task->StkLimitPtr,
                         task->StkSize,
    ...
    fprintf(stdout, "%s\n", "TEST 1");
    task->StkPtr = task->UsableStk;
    fprintf(stdout, "%s\n", "TEST 2");
    task->TaskState = OS_TASK_STATE_DLY;*/
    OS_RdyListRemove(task);
    OS_SkipListInsert(task);
    fprintf(stdout, "%s\n", "TEST 3");
    OSSched();
}
...
{
    int i;
    OS_TCB *originalNode = head.firstTCB, *currentTCB;
    for (i = SKIP_LIST_MAX_LVL - 1; i >= 0; i--)
    {
